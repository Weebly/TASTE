...
set_screen_resolution() {
  log "Configuring screen resolution..."
  execute "VBoxManage controlvm \"${vm_name}\" setvideomodehint 1280 1024 32"
  chk fatal $? "Could not set screen resolution."
}
...
set_guest_property() {
  log "Setting default APPLICATION_NAME property..."
  execute "VBoxManage guestproperty set \"${vm_name}\" APPLICATION_NAME default_application_name"
  chk error $? "Could not disable Firewall"
}
...
set_ie_config() {
  ...
  log "Disabling Network Location Wizard..."
  copyto disable_network_location_wizard.reg "${tools_path}" "${vm_temp}"
  execute "VBoxManage guestcontrol \"${vm_name}\" execute --image 'C:\\Windows\\Regedit.exe' --username 'IEUser' --password 'Passw0rd!' -- /s '${vm_temp}disable_network_location_wizard.reg'"
  chk error $? "Could not disable IE-Cache"
}
...
install_chrome() {
  ...
  execute "VBoxManage guestcontrol \"${vm_name}\" execute --image \"${vm_temp}${chrome_exe}\" --username 'IEUser' --password 'Passw0rd!'"
...
start_selenium_w7() {
  ...
  #copyto "selenium.bat" "${selenium_path}" 'C:/selenium/'
  chk error $? "Could not copy Selenium-Startup-File"
}
...
export_vm() {
  log "Exporting VM ${1}..."
  rm -f "/mnt/isi-weebly-live/automation/base_images/${vm_name}_selenium.ova"
  VBoxManage export "${vm_name}" -o "/mnt/isi-weebly-live/automation/base_images/${vm_name}_selenium.ova"
  chk skip $? "Could not export VM ${1}"
  chmod 0644 "/mnt/isi-weebly-live/automation/base_images/${vm_name}_selenium.ova"
}
